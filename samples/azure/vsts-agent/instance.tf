...
resource "terraform_data" "instance" {
  ...
  provisioner "local-exec" {
    command = format(
      "cherryctl server create --region %v --hostname %v --plan %v --image %v --ssh-keys %v %v %v --output json | jq -r .id | tee ${path.root}/.terraform/instance-id-${self.id}",
      local.instance_region,
      local.instance_name,
      local.instance_plan,
      local.instance_image,
      ...
      "", # "--spot-instance",
      "", # --userdata-file ~/src/gusztavvargadr/chef/samples/policies/vsts-agent/cloud-init.yml
    )
  }
