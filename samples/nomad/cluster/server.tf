...
resource "aws_s3_object" "server_server_cert" {
  bucket = aws_s3_bucket.default.bucket
  key    = "server/config/tls/server-cert.pem"
  source = "${path.root}/server/artifacts/config/tls/server-cert.pem"
}

resource "aws_s3_object" "server_server_key" {
  bucket = aws_s3_bucket.default.bucket
  key    = "server/config/tls/server-key.pem"
  source = "${path.root}/server/artifacts/config/tls/server-key.pem"
}
...
resource "terraform_data" "server_provision" {
  triggers_replace = [
    ...
    md5(file(local_file.server_provision.filename))
  ]
